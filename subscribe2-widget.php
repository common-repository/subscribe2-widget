...
class subscribe2_widget extends mijnpress_plugin_framework
{
	...
	{
		$widget = new subscribe2_widget();
		$options = $widget->get_defaults();
		
		$html = str_replace(array('[TITLE]','[FORM]'),array(stripslashes($options['title']),'[subscribe2]'),stripslashes($options['layout']));
	...
	function get_defaults()
	{
		...
		if (!is_array($options) || empty($options['layout']))
		{ 
		$options = array (
			'layout' => '<li id="subscribe2_widget" class="widget-container">'."\n".'<h3 class="widget-title">[TITLE]</h3>'."\n[FORM]\n</li>"
            );
		}			
		return $options;
	}
	...
	function admin_widget_controls() {
		...
		$options = array (
            ...
            if ($_POST['plugin__subscribe2_widget-submit']) {
            	...
            	$options['layout'] = (isset($_POST['layout']) && !empty($_POST['layout']));
...
<p><label for="layout">Layout:</label><br />Do not change if you do not know what you are doing. <br/>Remove everything to reset to defaults.
Use [TITLE] to show the title (see previous textarea) and [FORM] to show the input field.
<textarea style="width: 350px; height: 150px;" id="layout" name="layout"><?php echo stripslashes($options['layout']); ?></textarea>
